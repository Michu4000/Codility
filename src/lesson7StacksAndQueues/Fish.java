...
public class Fish {
	public static int solution(int[] A, int[] B) {
		...
		int N = A.length;
		boolean[] deadArray = new boolean[N]; // is fish dead or alive
		int res = N; // number of fish alive => result
		int prevRes = 0;
		...
		do {
			...
			for (int i = N - 2; i >= 0; i--) {
				if (B[i] == 1 && deadArray[i] == false) { // if fish is alive and swims downstream
					for (int j = i + 1; j < N; j++) {
						
						if(deadArray[j] == true)
							continue;
						
						if (B[j] == 0) { // if meets fish swimming opposite direction

							res--; // one of them will die...
						...
						if (B[j] == 1) // if meets fish swimming same direction => can't get through
							break;
					}
				}
	...
	public static void main(String[] args) {
		...
		System.out.println("A = [4, 3, 2, 1, 5], B = [0, 1, 0, 0, 0]");
		System.out.println(solution(new int[] { 4, 3, 2, 1, 5 }, new int[] { 0, 1, 0, 0, 0 }));
		System.out.println("");
		System.out.println("A = [5], B = [1]");
		System.out.println(solution(new int[] { 5 }, new int[] { 1 }));
		System.out.println("");
		System.out.println("A = [5], B = [0]");
		System.out.println(solution(new int[] { 5 }, new int[] { 0 }));
		System.out.println("");
		System.out.println("A = [5, 4], B = [1, 0]");
		System.out.println(solution(new int[] { 5, 4 }, new int[] { 1, 0 }));
		System.out.println("");
		System.out.println("A = [4, 3, 2, 1, 5], B = [0, 1, 0, 1, 0]");
		System.out.println(solution(new int[] { 4, 3, 2, 1, 5 }, new int[] { 0, 1, 0, 1, 0 }));
		System.out.println("");
		System.out.println("A = [4, 3, 5], B = [0, 1, 0]");
		System.out.println(solution(new int[] { 4, 3, 5 }, new int[] { 0, 1, 0 }));
		System.out.println("");
	}
